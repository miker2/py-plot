...
class DockedVisualizer3DWidget(DockedWidget):
    ...
    def set_source(self, source):
        self.widget()._3d_viz.set_source(source)

    @property
    def has_source(self):
        return self.widget()._3d_viz.has_source

    def update(self, tick):
        self.widget()._3d_viz.update_viz(tick)
...
class VisualizerWidget(QWidget):
    ...
    def _update_robot_model(self, idx):
        ...
        self._3d_viz.set_robot_model(urdf_path)


# Shape helpers:
def _create_arrow(color=(1., 1., 1., 1.), width=2, pos=(0, 0, 0), vec=(0, 0, 0)):
...
def _create_sphere(radius=0.05, color=(1., 0, 0, 1.), draw_faces=True, draw_edges=False):
...
class Visualizer3DWidget(GLViewWidget):
    def __init__(self, parent=None, source=None):
        ...
        self.p_com = _create_sphere(radius=0.01, color=(1., 0, 0, 1.))
        self.addItem(self.p_com)

        self.p_cop = _create_sphere(radius=0.01, color=(0, 0, 1., 1.))
        ...
        for i in range(len(_LEG_NAMES_)):
            self.foot_pos[i] = _create_sphere(radius=0.01, color=force_vec_color)
            self.addItem(self.foot_pos[i])
            self.force_vectors[i] = _create_arrow(color=force_vec_color)
        ...
        self.set_robot_model("robot.urdf")
    ...
    def set_robot_model(self, model_path):
        # Use pinocchio to update the kinematics
    ...
    def update_viz(self, tick):
        ...
        if not self.has_source:
            return
        if self._tick >= self._source.model().tick_max:
            self._tick = min(self._tick, self._source.model().tick_max)
            return

        # Cache the base position and orientation here since they'll be used in multiple places.
        for i in range(len(_DIR_)):
            self._base_pos[i] = self._get_value_at_tick(f"{_estimator_str_}.p_body_rt_world.{_DIR_[i]}")
        for i in range(len(_QUAT_)):
            self._wRb[i] = self._get_value_at_tick(f"imu.quat.{_QUAT_[i]}")
    ...
    def draw_force_vectors(self):
        ...
        for idx in range(len(self.force_vectors)):
            ...
            force = np.zeros(3)
            for i in range(len(_DIR_)):
                pos[i] = self._get_value_at_tick(pos_str.format(leg, _DIR_[i]))
                force[i] = self._get_value_at_tick(force_str.format(leg, _DIR_[i]))
            data = np.zeros((2, 3))
            data[0, :] = pos
    ...
    def draw_positions(self):
        ...
        p_cop = np.zeros(3)
        for i in range(len(_DIR_)):
            p_com[i] = self._get_value_at_tick(f"ctrl_data.p_com.{_DIR_[i]}")
            p_cop[i] = self._get_value_at_tick(f"p_cop_rt_world.{_DIR_[i]}")
    ...
    def update_kinematics(self):
        ...
        for leg in range(len(_LEG_NAMES_)):
            leg = _LEG_NAMES_[leg]
            ...
            self.leg_chains[leg].setData(pos=j_pos)
    ...
    def _get_value_at_tick(self, varname):
        return self._source.model().get_data_by_name(varname)[self._tick]
